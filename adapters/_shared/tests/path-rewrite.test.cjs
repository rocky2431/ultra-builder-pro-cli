...
test('resolveSkillRef routes to runtime-specific root under homeDir', () => {
  ...
  const kimi = resolveSkillRef('@skills/ultra-init/SKILL.md', { runtime: 'kimi', homeDir: home });
  ...
  assert.equal(kimi, path.join(home, '.kimi-code/skills/ultra-init/SKILL.md'));
...
test('rewriteWorkflowRefInText + resolveAssetTarget give predictable paths', () => {
  ...
  for (const rt of ['claude', 'opencode', 'codex', 'kimi']) {
