...
function loadPkgVersion(repoRoot) {
  try {
    ...
    // Fall back to 0.0.0 but surface the reason — silent swallow hid a
    // real "package.json moved" regression in a Phase 4 code review.
    process.stderr.write(`gemini adapter: loadPkgVersion fell back to 0.0.0 (${err.message})\n`);
    return '0.0.0';
  }
}
...
function buildManifest(repoRoot, extensionRoot) {
  ...
  return {
    ...
    // Sibling identification block — outside env so it cannot leak into the
    // MCP server's child process (P2 #9). Uninstall reads manifest._ubp.source.
...
function uninstall(ctx) {
  ...
  if (fs.existsSync(extensionRoot)) {
    // Manifest carries the _ubp sibling block — if tampered with, bail loud.
    const manifestFile = path.join(extensionRoot, 'gemini-extension.json');
    if (fs.existsSync(manifestFile)) {
      try {
        const manifest = JSON.parse(fs.readFileSync(manifestFile, 'utf8'));
        if (!manifest._ubp || manifest._ubp.source !== SOURCE_TAG) {
