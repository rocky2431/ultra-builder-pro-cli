...
1. Validate the immutable `WORKER_PACKET`, exact `PACKET_DIGEST`, `SESSION_PATH`,
   ...
   acceptance context. Stop incomplete when any value differs from the packet.
...
Do not modify tests or production code, call Ultra MCP write tools, change task state,
or edit another worker's artifact.
